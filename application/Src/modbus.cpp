#include "main.h"
#include "modbus.h"
#include "hicode.h"
#include "usart.h"
#include "retarget.h"
#include "bsp_usart.h"

const uint8_t table_crc_hi[] = {
        };


const uint8_t table_crc_lo[] = {
        };

//DataLen是需要计算的数据长度（字节数）
uint16_t modbus::crc16(uint8_t *buffer, uint16_t DataLen){
    uint8_t CRC_H = 0xFF;
    uint8_t CRC_L = 0xFF;
    uint32_t uIndex;
    while (DataLen--){
        uIndex = CRC_H ^ *buffer++;
        CRC_H = CRC_L ^ table_crc_hi[uIndex];
        CRC_L = table_crc_lo[uIndex];
    }
    return ((CRC_H<< 8)	|	(CRC_L));
}


MODBUS_ERROR modbus::Modbus_ReceiveHandler(sensor& soil) const
{
    uint8_t data_length = Buffer[2]; //获取数据长度

    //将8bit的crc组合成16bit                           CRC_H|CRC_L
    uint16_t rec_crc = (Buffer[data_length+3] << 8) | Buffer[data_length+4]; //获取数据帧里的crc校验码。
    if(Buffer[0] == 0x01){
        //算一下rcr
        uint16_t calCRC = crc16(Buffer, data_length+3);
        if(rec_crc == calCRC){  //核验crc
            switch(Buffer[1]){   //根据功能码选择相应的处理函数。
                case 0x03:
                    sensor::Function_03_Handler(Buffer+3, soil);
                    break;
                default:
                    BON_DEBUG("code error");
                    return CODE_ERROR;  //功能码错误
            }
        }else {
            BON_DEBUG("crc error");
            return CRC_ERROR;   //CRC校验错误
        }
    }else {
        BON_DEBUG("id error");
        return ID_ERROR;    //从机ID错误
    }
    BON_DEBUG("modbus ok");
    return MODBUS_OK;   //没有错误
}

//读取传感器
MODBUS_ERROR modbus::Modbus_Transmit(sensor& soil){
    HAL_UART_Transmit(&MODBUS_SER, soil.mb_message, sizeof(soil.mb_message), 10);
    return MODBUS_OK;
}
